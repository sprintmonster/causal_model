...
신문 OCR 텍스트에서 추출한 지식그래프를 바탕으로 인과모델을 만들고, 금주법(Prohibition) 관련 사건을 어떤 요인이 이끌었는지 분석하는 것이 목적입니다.

## 데이터와 파이프라인

- 원천 데이터: `data/1916` ~ `data/1920` 아래의 OCR 텍스트 파일
- 추출 단계: 텍스트 배치를 대상으로 한 LLM 기반 지식그래프 추출( RTX 4090, Qwen 2.5 7B LLM을 사용 )
